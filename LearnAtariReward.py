...
def create_training_data(c_half,cone,c_5e,difs,learning_rewards,demonstrations, num_trajs, num_snippets, min_snippet_length, max_snippet_length):
    ...
    for n in range(num_trajs):
        ...
        diff = (learning_rewards[ti] - learning_rewards[tj])
        difs.append(diff)
        if 200 <= diff:
            label = 5e-5
            c_5e+=1
    ...
    for n in range(num_snippets):
        ...
        traj_i = demonstrations[ti] #skip everyother framestack to reduce size
        traj_j = demonstrations[tj]
        max_traj_length = max(max_traj_length, len(traj_i), len(traj_j))
        difs.append(diff)
        diff = (learning_rewards[ti]-learning_rewards[tj])
    ...
    return c_half,cone,c_5e,difs,training_obs, training_labels
...
def learn_reward(reward_network, optimizer, training_inputs, training_outputs, num_iter, l1_reg, checkpoint_dir):
    ...
    for epoch in range(num_iter):
        ...
        for i in range(len(training_labels)):
            ...
            #print('Outputs:', nn.functional.softmax(outputs,dim=0,dtype=torch.double))
            loss = loss_criterion(nn.functional.softmax(outputs,dim=0,dtype=torch.double)[1].unsqueeze(0), labels) + l1_reg * abs_rewards
            loss.backward()
            optimizer.step()
...
def calc_accuracy(reward_network, training_inputs, training_outputs):
    ...
    with torch.no_grad():
        for i in range(len(training_inputs)):
            ...
            f = lambda x: torch.sign(torch.tensor([x-0.5])).item()
            pred_label=torch.nn.functional.softmax(outputs,dim=0)[1]
            #_, pred_label = torch.max(outputs,0)
            if f(pred_label) == f(label):
                num_correct += 1.
...
if __name__=="__main__":
    ...
    cone = 0
    c_5e=0
    c_half, cone, c_5e, difs, training_obs, training_labels = create_training_data(c_half, cone, c_5e, difs, learning_returns, demonstrations, num_trajs, num_snippets, min_snippet_length, max_snippet_length)
    print("Start",c_half,"c_half",cone,"cone",c_5e,"c_5e","end")
    print("num training_obs", len(training_obs))
    print("num_labels", len(training_labels))
    
    returns = pd.DataFrame(difs)    
    fig, ax = plt.subplots()
    returns.hist(ax=ax,bins = 40 )
    plt.xlabel("diff")
    plt.ylabel("Frequency")
    #tf.print(x)
    fig.savefig('_ret.png')
